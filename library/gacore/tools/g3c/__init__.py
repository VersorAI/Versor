...
import numba
import numpy as np
from gacore.tools.g3 import quaternion_to_rotor, random_euc_mv, \
    random_rotation_rotor, generate_rotation_rotor
from gacore.g3c import *
import gacore as cf
from gacore import NUMBA_PARALLEL, MVArray
...
# Allow syntactic alternatives to the standard included in the gacore package
ninf = einf
no = -eo
...
def interpret_multivector_as_object(mv):
    ...
    Similar to :func:`gacore.tools.classify.classify`, although that function
...
@numba.njit
def norm(mv):
    """ Alias of :meth:`gacore.MultiVector.__abs__` """
    return abs(mv)
...
@numba.njit
def normalised(mv):
    """ Alias of :meth:`gacore.MultiVector.normal` """
    return mv.normal()
