...
from gacore import Cl, randomMV, Frame, \
...
class TestClifford:
    ...
    def test_pseudoscalar(self, algebra):
        ...
        """
        layout, blades = algebra, algebra.blades
        ps = reduce(gacore.op, layout.blades_of_grade(1))
    ...
    def test_exp_g4(self, g4):
        ...
        generated by results of gacore v0.82
        '''
        layout, blades = g4, g4.blades

        valB = np.array([-0.0, 0.0, 0.0, -1.9546896043012914, -0.0, 0.7069828848351363, 1.0226966962560002, -0.0, -0.0, -0.22839793693302957, 1.8673816483342143, -0.0, -1.7694566455296474, 0.0, -0.0, -0.0])
        valexpB = np.array([-0.8154675764311629, 0.0, 0.0, 0.3393508714682218, 0.0, 0.22959588097548828, -0.01536404898029994, 0.0, 0.0, -0.1331099867581965, 0.012688721722814184, 0.0, 0.35678394795928464, 0.0, 0.0, -0.14740840378445502])
    ...
    def test_inv_g4(self, g4):
        ...
        '''
        layout, blades = g4, g4.blades
        valA = np.array([-0.3184271488037198, -0.8751064635010213, -1.5011710376191947, -0.3297631748225678, 1.7946332649746224, 0.04310366054166925, -1.545423393858595, -1.32279679741638, -0.8899576254164621, 1.3970365638677635, 1.7790215501876614, 0.5874769077573831, 0.4785341530609175, -1.0227287710873676, 1.779673249468527, -1.5415648119743852])
        valAinv = np.array([0.06673424072253006, -0.005709960252678998, -0.10758540037163118, 0.04123255613093294, 0.1805895938775471, -0.015395162562329407, -0.1462160646855434, 0.1396713851886765, 0.13919236400967427, -0.1388977308136247, -0.1183453106997158, -0.02572904638749348, -0.06961956152268277, 0.02079613649197489, -0.06933660606043765, -0.05436077710009021])
    ...
    def test_basis(self, g4):
        ...
        for i in range(len(vectors)):
            blade = reduce(gacore.operator.op, vectors[i:])
            basis = blade.basis()
            roundtrip = reduce(gacore.operator.op, basis)
...
class TestPrettyRepr:
    ...
    def test_layout(self, g3):
        ...
        Layout([1, 1, 1],
               ...
               order=BasisBladeOrder.bitmap(3),
               names=['', 'e1', 'e2', 'e12', 'e3', 'e13', 'e23', 'e123'])""")
    ...
    def test_multivector(self, g2):
        ...
        MultiVector(Layout([1, 1],
                           ...
                           order=BasisBladeOrder.bitmap(2),
                           names=['', 'e1', 'e2', 'e12']),
                    [0, 1, 2, 3],
                    dtype=int32)""")
    ...
    def test_multivector_predefined(self):
        ...
        from gacore.g2 import layout as g2
        ...
        # float is implied
        expected_i = "gacore.g2.layout.MultiVector([0, 1, 2, 3], dtype=int32)"
        expected_f = "gacore.g2.layout.MultiVector([0.0, 1.0, 2.0, 3.0])"
