"""
Tests of the ``gacore.g*`` modules and friends.
...
def test_import(alg_name):
    ...
    assert 'gacore.{}'.format(alg_name) not in sys.modules, "already tested elsewhere"

    importlib.import_module('gacore.{}'.format(alg_name))
...
def test_already_imported(alg_name):
    ...
    assert 'gacore.{}'.format(alg_name) in sys.modules
