"""
.. currentmodule:: gacore.transformations

=================================================
transformations (:mod:`gacore.transformations`)
=================================================
...
class FixedLayout(Transformation):
    ...
    Parameters
    ----------
    layout_src : :class:`~gacore.Layout` of S dimensions
        The layout from which this transformation takes multivectors as input
    layout_dst :  :class:`~gacore.Layout` of D dimensions
...
class LinearMatrix(FixedLayout, Linear):
    ...
    layout_src : ~gacore.Layout of S dimensions
        Passed on to :class:`FixedLayout`.
    layout_dst : ~gacore.Layout of D dimensions
        Passed on to :class:`FixedLayout`.

    See Also
    --------
    gacore.BladeMap : A faster but less general approach that works on basis blades
    ...
    def from_function(cls, func: Callable[[MultiVector], MultiVector], layout_src: Layout, layout_dst: Layout = None) -> 'LinearMatrix':
        ...
        >>> from gacore import transformations, Layout
        >>> l = Layout([1, 1])
        ...
        gacore.linear_operator_as_matrix : a lower-level function for working with a subset of basis blades
        """
        blades_dst = [
...
def between_basis_vectors(layout_src: Layout, layout_dst: Layout, mapping: Dict[Any, Any]=None) -> OutermorphismMatrix:
    ...
    Parameters
    ----------
    layout_src : ~gacore.Layout
        Passed on to :class:`FixedLayout`.
    layout_dst : ~gacore.Layout
        Passed on to :class:`FixedLayout`.
    mapping :
