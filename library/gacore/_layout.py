...
class Layout(object):
    ...
    order : Optional[BasisBladeOrder]
        ...
        .. warning::
            Various tools within gacore assume this default, so do not change
    ...
    def __init__(self, *args, **kw):
        ...
        try:
            ...
            if order is None:
                order = BasisBladeOrder.bitmap(len(sig))
    ...
    def gmt_func(self):
        from . import kernel
        return lambda a, b: kernel.geometric_product(a, b, self.sig)

    @_cached_property
    def imt_func(self):
        from . import kernel
        return lambda a, b: kernel.inner_product(a, b, self.sig)

    @_cached_property
    def omt_func(self):
        from . import kernel
        return lambda a, b: kernel.wedge_product(a, b, self.sig)

    @_cached_property
    def lcmt_func(self):
        from . import kernel
        return lambda a, b: kernel.left_contraction(a, b, self.sig)

    @_cached_property
    def left_complement_func(self):
        # This one might use the generated one or we need a kernel comp func
    ...
    def adjoint_func(self):
        from . import kernel
        return lambda x: kernel.reverse(x, self.sig)
    ...
    def randomMV(self, n=1, **kwargs) -> MultiVector:
        ...
        see `gacore.randomMV` for details
        '''
