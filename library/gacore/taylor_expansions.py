"""
.. currentmodule:: gacore.taylor_expansions

=====================================================
taylor_expansions (:mod:`gacore.taylor_expansions`)
=====================================================
...
.. note::
    Many of these functions are also exposed as :class:`~gacore.MultiVector` methods,
    such as :meth:`gacore.MultiVector.sin`. This means that ``mv.sin()`` or even ``np.sin(mv)`` can be used
    as a convenient interface to functions in this module, without having to import it directly.

    For example::

        >>> from gacore.g3 import *
        >>> import numpy as np
        >>> np.sin(np.pi*e12/4)
        (0.86867^e12)
