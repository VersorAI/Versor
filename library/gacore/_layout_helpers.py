...
class BasisBladeOrder:
    ...
    @classmethod
    def bitmap(cls, n_vectors: int) -> 'BasisBladeOrder':
        """
        Get a bitmap ordering.
        
        This sorts basis blades by their integer bitmap value.
        This provides O(1) mapping between index and bitmap.
        """
        return _BitmapBasisBladeOrder(n_vectors)


class _BitmapBasisBladeOrder(BasisBladeOrder):
    def __init__(self, n_vectors: int):
        self._n = n_vectors
        self.index_to_bitmap = np.arange(2**n_vectors, dtype=int)
        self.bitmap_to_index = np.arange(2**n_vectors, dtype=int)
        self.grades = np.zeros(2**n_vectors, dtype=int)
        # Precompute grades
        for i in range(2**n_vectors):
            self.grades[i] = count_set_bits(i)

    def __repr__(self):
        return 'BasisBladeOrder.bitmap({})'.format(self._n)

    def __eq__(self, other):
        if self is other:
            return True
        if not isinstance(other, _BitmapBasisBladeOrder):
            return NotImplemented
