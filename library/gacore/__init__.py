...
spelling for features which have always been in ``gacore``.
...
==============================
These functions are used to change the global behavior of ``gacore``.
...
from gacore.io import write_ga_file, read_ga_file  # noqa: F401
...
def grades_present(objin: 'MultiVector', threshold=0.0000001) -> Set[int]:
    ...
    warnings.warn(
        "`gacore.grades_present(x)` is deprecated, use `x.grades()` instead. "
        "Note that the method uses `gacore.eps()` as the default tolerance.",
...
def Cl(p: int = 0, q: int = 0, r: int = 0, sig=None, names=None, firstIdx=1,
    ...
    sig
        See the docs for :class:`gacore.Layout`. If ``sig`` is passed, then
        `p`, `q`, and `r` are ignored.
    names, firstIdx
        See the docs for :class:`gacore.Layout`.
...
def conformalize(layout: Layout, added_sig=[1, -1], *, mvClass=MultiVector, **kwargs):
    ...
    Examples
    ---------
    >>> from gacore import Cl, conformalize
    >>> G2, blades = Cl(2)
